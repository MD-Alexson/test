    ...
    <section class="cabinet-content">
        ...
        <div class="content-main">
            ...
            <div class="add-material">
                <form action="{{ action('Backend\PaymentsController@update', ['payment_id' => $payment->id]) }}" method="post" enctype="multipart/form-data">
                    {{ csrf_field() }}
                    <fieldset>
                        ...
                        <div class="add-mat-right merch_id" style='display: none'>
                            <input type="text" class="input" placeholder="Введите ID" name="merch_id" maxlength="24" value="{{ $payment->merch_id }}" required="required">
                        </div>
                        <div class="add-mat-left">
                            <div class="add-mat-text tooltip-holder">Пароль</div>
                        </div>
                        <div class="add-mat-right">
                            <input type="text" class="input" placeholder="Введите пароль" name="key" required="required" maxlength="128" value="{{ $payment->key }}">
                        </div>
                        <div class="add-mat-left">
                        ...
                        <div class="add-mat-right">
                            <input type="text" class="input" placeholder="Введите id" name="item_id" required="required" maxlength="40" value="{{ $payment->item_id }}">
                        </div>
                        <div class="add-mat-left">
                        ...
                        <div class="add-mat-right">
                            <div class="check-list">
                                <div class="check-block">
                                    ...
                                    <input name="membership" type="checkbox" class="check" id="ch2" checked="checked">
                                    @else
                                    ...
                                    <br/>
                                    <br/>
                                    <input type="number" class="input inline-button" placeholder="Число" name="membership_num" value="{{ $payment->membership_num }}" min="1" style="width: 80px;">
                        ...
                        <div id="optional">
                            ...
                            <div class="add-mat-right">
                                ...
                                <input type="text" class="input input-long" name="subject" required="required" maxlength="255" value="{{ $payment->subject }}">
                                ...
                                <textarea name="message" class="textarea">{{ $payment->message }}</textarea>
                            </div>
                            <div class="add-mat-left">
                            ...
                            <div class="add-mat-right">
                                <p class='tooltip-holder'>Тема письма: <span class="tooltip-icon tooltip-icon-inline" data-content="<div class='popover-inner-text'>Если оставить поле пустым, письма пользователям отправлены не будут.<br/><br/>Список доступных переменных:<br/>{username} - Имя пользователя<br/>{pass_link} - Ссылка на создание\напоминанние пароля<br/>{email} - email пользователя<br/>{project_name} - Название вашего проекта<br/>{level_name} - Название уровня доступа</div>">?</span></p>
                                <input type="text" class="input input-long" name="subject2" required="required" maxlength="255" value="{{ $payment->subject2 }}">
                                ...
                                <textarea name="message2" class="textarea">{{ $payment->message2 }}</textarea>
                            </div>
                        </div>
                        <br/>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        var membership_type = "{{ $payment->membership_type }}";
        $("select[name=method]").val(method);
        $("select[name=membership_type]").val(membership_type);
        $("select[name=level_id]").val("{{ $payment->level_id }}");
        ...
        } else if(method === 'Justclick'){
            $("#changeble_note").html(chnote2);
        } else if(method === 'Fondy'){
